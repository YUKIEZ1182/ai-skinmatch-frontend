...
export default function AuthModal({ isOpen, onClose, onLoginSuccess }) {
  ...
  const showBack = mode === 'register_step1' || mode === 'register_step2' || mode === 'forgot_password';
  ...
  // ✅ เช็คอีเมลซ้ำตั้งแต่ step1 คืนมาแล้ว
  ...
  const handleForgotPassword = async (e) => {
    ...
    try {
      ...
      if (!res.ok) throw new Error('ไม่สามารถส่งลิงก์รีเซ็ตได้');

      setSuccessMessage('กรุณาตรวจสอบอีเมลของคุณ');
      setMode('login');
    } catch (err) {
    ...
    <div className="auth-modal-overlay" onClick={onClose}>
      <div className="auth-modal-content" onClick={(e) => e.stopPropagation()}>
        <div className="auth-modal-header">
          ...
          {/* เอาปุ่ม X ปิดหน้าต่างออกตรงนี้ */}
          <div className="auth-modal-header-slot"></div>
        </div>
