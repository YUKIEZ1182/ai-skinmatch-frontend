...
export default function AuthModal({ isOpen, onClose, onLoginSuccess }) {
  ...
  // 🔥 เพิ่ม State สำหรับเก็บ Error Message ตอน Login
  const [loginError, setLoginError] = useState('');
  const [isLoading, setIsLoading] = useState(false);

  useEffect(() => {
    if (isOpen) {
       setLoginError(''); // Reset error เมื่อเปิด Modal ใหม่
       setErrors({});
    }
  }, [isOpen]);
  ...
  const handleEmailChange = (e) => {
    ...
    setEmail(val);
    setLoginError(''); // เคลียร์ Error Banner เมื่อผู้ใช้เริ่มพิมพ์ใหม่
  ...
  // ... (ส่วน handleConfirmPasswordChange และ Logic เช็ค Password เหมือนเดิม)
  ...
  const handleFinalSubmit = async (e) => {
    e.preventDefault();
    setIsLoading(true);
    setLoginError(''); // Reset error ก่อนยิง API
      ...
      if (mode === 'login') {
        ...
        if (response.ok) {
          ...
          if (MEMBER_ROLE_ID && meData.data.role !== MEMBER_ROLE_ID) {
            setLoginError("บัญชีนี้ไม่มีสิทธิ์เข้าใช้งาน (Role ไม่ถูกต้อง)");
            setIsLoading(false);
            return; 
          }
          ...
          // 🔥 ใช้ Banner แทน alert()
          // ถ้า error code เป็น invalid credentials หรือ user not found ให้แนะนำสมัครสมาชิก
          console.error("Login Error:", data);
          setLoginError("อีเมลหรือรหัสผ่านไม่ถูกต้อง กรุณาตรวจสอบอีกครั้ง หรือสมัครสมาชิกใหม่หากยังไม่มีบัญชี");
        }
      } 
      
      // --- REGISTER ---
      else if (mode === 'register_step2') {
        // ... (Logic Register เหมือนเดิม)
        if (!birthDate || !gender || !skinType) {
            alert("กรุณากรอกข้อมูลส่วนตัวให้ครบถ้วน"); // Register ใช้ alert ก็ได้ หรือจะเปลี่ยนเป็น error state ก็ได้
            setIsLoading(false);
            return;
        }
        ...
        if (response.ok) {
          setMode('register_success');
           ...
           // กรณี Register อาจจะยังใช้ alert หรือจะทำ Error state แยกก็ได้
           if (data.errors?.[0]?.message?.includes("forbidden")) {
             alert("เกิดข้อผิดพลาด: Role permission denied");
          } else {
             alert(`สมัครสมาชิกไม่สำเร็จ: ${data.errors?.[0]?.message || 'อีเมลนี้อาจถูกใช้งานแล้ว'}`);
          }
        }
      }
    } catch (error) {
  ...
  return (
    <div className="auth-modal-overlay">
      <div className="auth-modal-content">
        ...
        {mode === 'login' && (
          <>
            <h2 className="auth-title">
                {hideRegisterButton ? "เข้าสู่ระบบ" : "กรุณาสมัครสมาชิกหรือเข้าสู่ระบบก่อนดำเนินการต่อ"}
            </h2>
            ...
            {/* 🔥 Alert Banner สำหรับ Login Error */}
            {loginError && (
                <div style={{
                    background: '#f8d7da', 
                    color: '#721c24', 
            ...
            <form onSubmit={handleFinalSubmit}>
              <div className="form-group">
                ...
                <input 
                    type="text" 
                    className={`auth-input ${loginError ? 'input-error' : ''}`} // ไฮไลท์แดงถ้ามี error
                    placeholder="name@example.com" 
                    value={email} 
                    onChange={handleEmailChange} // ใช้ handler ใหม่ที่เคลียร์ error
                />
              </div>
              <div className="form-group">
                <label className="form-label">รหัสผ่าน</label>
                <div className={`password-wrapper ${loginError ? 'input-error' : ''}`} style={{position: 'relative'}}>
                  <input 
                    type={showPassword ? "text" : "password"} 
                    className="auth-input" 
                    placeholder="กรอกรหัสผ่านของคุณ" 
                    value={password}
                    onChange={handlePasswordChange} // ใช้ handler ใหม่ที่เคลียร์ error
                  />
              ...
              {!hideRegisterButton && (
                  <button type="button" className="auth-outline-btn" onClick={handleGoToRegister}>สมัครสมาชิกใหม่</button>
              )}
            </form>
          </>
        )}

        {/* ---------------- REGISTER STEP 1 (เหมือนเดิม) ---------------- */}
        {mode === 'register_step1' && (
          <>
            ...
            <form onSubmit={handleNextStep}>
              <div className="form-group">
                ...
                />
                <div style={{fontSize: '0.75rem', color: '#888', marginTop: '4px', marginLeft: '2px'}}>
                    เราจะใช้สำหรับการเข้าสู่ระบบและแจ้งเตือน
                </div>
          ...
          <>
            ...
            <form onSubmit={handleFinalSubmit}>
              ...
              <div className="form-group">
                ...
                <CustomSelect options={genderOptions} placeholder="เลือกเพศ" value={gender} onChange={setGender} />
              </div>
              <div className="form-group">
                <div style={{display: 'flex', justifyContent: 'space-between', alignItems: 'center'}}>
                    ...
                    <a 
                        ...
                        target="_blank" 
                        rel="noreferrer" 
                        style={{fontSize: '0.8rem', color: '#F1978C', textDecoration: 'none', display: 'flex', alignItems: 'center', gap: '4px'}}
                    >
                ...
                <CustomSelect options={skinTypeOptions} placeholder="เลือกสภาพผิว" value={skinType} onChange={setSkinType} />
              </div>
              <button type="submit" className="auth-submit-btn" disabled={isLoading}>
        ...
        {/* ---------------- SUCCESS MODE (เหมือนเดิม) ---------------- */}
        {mode === 'register_success' && (
            <div style={{textAlign: 'center', padding: '20px 0'}}>
                <div style={{
                    width: '80px', height: '80px', background: '#d4edda', borderRadius: '50%', 
                    display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 20px'
                }}>
                    <svg width="40" height="40" viewBox="0 0 24 24" fill="none" stroke="#28a745" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round"><polyline points="20 6 9 17 4 12"></polyline></svg>
                </div>
