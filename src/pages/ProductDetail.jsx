...
export default function ProductDetail({ onAddToCart }) {
      ...
      try {
        ...
        if (json.data) {
          ...
          if (item.illustration && item.illustration.length > 0) {
             images = item.illustration.map(img => 
                img.directus_files_id ? `${API_URL}/assets/${img.directus_files_id}` : null
             ).filter(Boolean);
          }
  ...
  useEffect(() => {
    const fetchRelated = async () => {
      try {
        const response = await fetch(`${API_URL}/items/product?limit=4&filter[id][_neq]=${id}&fields=id,name,price,thumbnail,categories.category_id.name`);
        const json = await response.json();
        if (json.data) {
           const mappedRelated = json.data.map(p => ({
             ...
             image: p.thumbnail ? `${API_URL}/assets/${p.thumbnail}` : 'https://via.placeholder.com/300x300?text=No+Image',
    ...
    <div className="product-detail-page">
      ...
      {relatedProducts.length > 0 && (
        <div className="related-section">
            <h2 className="related-title">สินค้าแนะนำอื่นๆ</h2>
